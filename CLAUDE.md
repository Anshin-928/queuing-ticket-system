...
## 整理券PDF生成フェーズの設計メモ（`/admin/[booth_id]/tickets`）

### 発券と PDF 生成の関係

**DBスキーマの変更は不要。** 現行スキーマで以下のフローを実現できる。

#### 発券ロジック（実装済み）

`crowded` モードでの発券は以下の優先順で動作する：

1. `unissued` チケットが存在する場合 → そのチケットを `waiting` に更新（QR印刷済み想定）
2. `unissued` が存在しない場合 → `max(ticket_number) + 1` で新規 `waiting` を INSERT

これにより PDF 生成前でも発券可能。

#### PDF 生成ロジック（未実装・設計メモ）

スタッフが「1〜N番まで印刷」を指定したとき：

```
for i in 1..N:
  既存レコードあり（waiting/called/done/unissued）
    → そのレコードの tickets.id（UUID）をQRコードに使用
  既存レコードなし
    → status='unissued' で新規 INSERT → そのUUIDをQRコードに使用
```

**結果:** 発券済みの番号もPDF生成後の番号も、同一のUUIDで一意に紐づく。

#### 使用パッケージ
- `@react-pdf/renderer` でA4サイズに面付け出力（未インストール、実装時に追加）
